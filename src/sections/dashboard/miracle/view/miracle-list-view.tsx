...
import { useMiracles } from 'src/hooks/use-miracles';
...
export function MiracleListView() {
  const { miracles, loading } = useMiracles();

  const [sortBy, setSortBy] = useState('latest');

  const { state, setState } = useSetState<IPostFilters>({ publish: 'all' });

  const dataFiltered = applyFilter({ inputData: miracles, filters: state, sortBy });
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        ...
        action={
          <Button
            ...
            href={paths.dashboard.miracle.new}
            variant="contained"
      ...
      >
        <PostSearch redirectPath={(title: string) => paths.dashboard.miracle.details(title)} />
        ...
        {['all', 'published', 'draft'].map((tab) => (
          <Tab
            ...
            icon={
              ...
              >
                {tab === 'all' && miracles.length}
                {tab === 'published' &&
                  miracles.filter((miracle) => miracle.publish === 'published').length}
                {tab === 'draft' &&
                  miracles.filter((miracle) => miracle.publish === 'draft').length}
              </Label>
            }
      ...
      <PostListHorizontal miracles={dataFiltered} loading={loading} />
    </DashboardContent>
  );
}
